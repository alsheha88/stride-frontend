...
const tierColors = [
  { filled: "text-danger", rating: "Beginner" },
  { filled: "text-secondary", rating: "Learning" },
  { filled: "text-primary", rating: "Confident" },
  { filled: "text-success", rating: "Strong" },
  { filled: "text-tertiary", rating: "Mastered" },
];
...
const ConceptDetailPage = () => {
  ...
  return (
    <main className="min-h-dvh flex px-3 md:px-8 pt-4 pb-16 max-w-6xl mx-auto">
      <Card>
        <div className="grid gap-10 h-full">
          ...
          <div className="flex items-center gap-4 justify-between flex-wrap">
            ...
            <p className="flex items-center gap-1 text-sm md:text-base">
              ...
              <span className={tier?.filled ?? "text-paragraph"}>
                {tier?.rating ?? "—"}
              </span>
            </p>
            <p className="flex items-center gap-1 text-sm md:text-base">
          ...
          {concept.projectLinks.length > 0 && (
            <div className="flex flex-col gap-4">
              <h4 className="text-paragraph text-lg md:text-xl font-semibold">
                Linked Projects
              </h4>
            ...
            {concept.notes.length > 0 ? (
              <ol className="text-paragraph text-sm md:text-base list-decimal mx-4">
                {concept.notes.map((note) => (
                  <li key={note.id} className="leading-loose">
                    {note.content}
                  </li>
                ))}
              </ol>
